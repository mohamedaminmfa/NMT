...
#{
    ...
    #{  
        ...
        #{
            actual_val_monitor_list, predicted_val_monitor_list = list(), list()
            actual_val_monitor_fileName     = checkpoint_dir + "eval_val_monitor_actual.txt"
            predicted_val_monitor_fileName  = checkpoint_dir + "eval_val_monitor_predicted.txt"
            actual_val_monitor_outputs, predicted_val_monitor_outputs  = list(), list()

            actual_train_monitor_list, predicted_train_monitor_list = list(), list()
            actual_train_monitor_fileName     = checkpoint_dir + "eval_train_monitor_actual.txt"
            predicted_train_monitor_fileName  = checkpoint_dir + "eval_train_monitor_predicted.txt"
            actual_train_monitor_outputs, predicted_train_monitor_outputs  = list(), list()
            ...
            #{			
                train_npy_list = []
                for fid in train_input:
                #{								
                    #file_id  = file_prefixs + str(int(fid.numpy()))
                    file_id  =  clean_tag( tensor_to_sentence(fid, inp_lang_train) )
                    file_loc = ""

                    for dir in file_dir:					
                        if os.path.exists( dir + file_dic[file_id] + file_extension ):
                            file_loc =  dir
                            break

                    #if file_loc != "" :
                    nyp_file = np.load(file_loc + file_dic[file_id] + file_extension)
                    train_npy_list.append( nyp_file )
                #}

                train_input = tf.convert_to_tensor(train_npy_list, dtype=tf.float32)			
                ...
                variables = model_inst.encoder.variables + model_inst.decoder.variables        
            ...
            #{   
                ...
                #{				
                    ...
                    #{
                        ...
                        #if file_loc != "" :
                        val_npy_list.append( np.load(file_loc + file_dic[file_id] + file_extension) )
                    #}

                    val_input = tf.convert_to_tensor(val_npy_list, dtype=tf.float32)	
                ...
                #{
                    ...
                    saver_info = "-EP:{}-LR:{}-TRAIN_LOSS:{:.4f}-VAL_LOSS:{:.4f}-"\
                                ...
                                "TRAIN_BLEU:{:.4f}-VAL_BLEU:{:.4f}-TRAIN_ROUGE:{:.4f}-VAL_ROUGE:{:.4f}".format(  TOTAL_EPOCHS, model_inst.optimizer_inst.learning_rate,
            ...
            #{
                ...
                #{
                    ...
                    saver_info = "-EP:{}-LR:{}-TRAIN_LOSS:{:.4f}-TRAIN_ACC:{:.4f}-TRAIN_BLEU:{:.4f}-TRAIN_ROUGE:{:.4f}".format(  TOTAL_EPOCHS, model_inst.optimizer_inst.learning_rate,
