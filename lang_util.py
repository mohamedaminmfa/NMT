...
from SL.lib.NMT_lib.bleu import *
from SL.lib.NMT_lib.rouge import *
from SL.lib.NMT_lib.text_processing_util import *

#...........................................................................................................................................................

def loadCheckpoint(checkpoint, checkpoint_dir, checkpoint_fileName=None, enable=True):
#{
    if checkpoint_fileName != None:
        
        data = 'model_checkpoint_path: "' + checkpoint_fileName + '"\nall_model_checkpoint_paths: "' + checkpoint_fileName + '"'
...
def create_dataset_pairs(dataset, Lfrom=0, Lto=-1, number_examples=-1):
#{ 
    dataset = read_corpus(dataset, number_examples)   

    if(Lto == -1):
        dataset = dataset
    else:
        dataset = dataset[Lfrom: Lto]
...
#{    
    def __init__(self, lang, min_fw, padding_enable):
    #{
        self.lang          = lang
        self.word2idx      = {}
        self.idx2word      = {}
    ...
    #{
        ...
        #{
            ...
            for index, word in enumerate(self.vocab):
                self.word2idx[word] = index + count
...
def load_dataset(dataset, inp_lang= None, targ_lang= None, Tx=None, Ty=None, seed= None, inp_min_fw=0, targ_min_fw=0, reverse_src_targ=False, Lfrom=0, Lto=-1, padding_enable=True):
#{    
    pairs = create_dataset_pairs(dataset, Lfrom, Lto)
    ...
    #{
        ...
        if reverse_src_targ:
            inp_lang  = LanguageIndex( (targ for src, targ in pairs), inp_min_fw, padding_enable )
            targ_lang = LanguageIndex( (src  for src, targ in pairs), targ_min_fw, padding_enable )
        else:
            inp_lang  = LanguageIndex( (src  for src, targ in pairs), inp_min_fw, padding_enable )
            targ_lang = LanguageIndex( (targ for src, targ in pairs), targ_min_fw, padding_enable )            
    #}
    ...
    if Tx is None:
        Tx = max_length_inp
        inp_lang.minLength  = min_length_inp
        inp_lang.maxLength  = max_length_inp

    if Ty is None:         
        Ty = max_length_targ
        targ_lang.minLength = min_length_targ
        targ_lang.maxLength = max_length_targ
...
def search_best_seed(path_to_file, number_examples, dataset_split, seed_from, seed_to, REVERSE_SRC_TARG):
    ...
    for i in range(seed_from, seed_to+1):
        ...
        data_partition = data_partitioning()
        training_dataset, validation_dataset, test_dataset = data_partition(corpus, dataset_split)
        ...
        input_OOV_train, target_OOV_train = load_dataset(training_dataset, inp_min_fw=0, targ_min_fw=0, reverse_src_targ=REVERSE_SRC_TARG)
        ...
        min_length_inp_val, min_length_targ_val,\
        input_OOV_val, target_OOV_val = load_dataset(validation_dataset, 
                                                    inp_lang_train, targ_lang_train,
                                                    Tx=max_length_inp_train, Ty=max_length_targ_train, seed= None, reverse_src_targ=REVERSE_SRC_TARG)


        if(input_OOV_val <= best_input_OOV):
            best_input_OOV = input_OOV_val
            best_seed_input_dic[i]  = best_input_OOV

        if(target_OOV_val <= best_target_OOV):
            best_target_OOV = target_OOV_val
            best_seed_targ_dic[i]   = best_target_OOV

        ALL_OOV = input_OOV_val + target_OOV_val
        ...
        if(i%100 == 0):            
            ...
            print(seed_info)
            save_file("/content/drive/My Drive/seed_search/seed_search.txt", seed_info, writeMode= "a")
