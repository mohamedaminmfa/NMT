...
class LuongsAttention(tf.keras.Model):
  ...
  def __init__(self, hparams, num_layers, debug_mode=False):
    ...
    self.W = tf.keras.layers.Dense( hparams.num_units )
  ...
  def  call(self, H, EO):  # H: Hidden State   , EO: Encoder Output  
    ...
    if self.rnn_cell_type.upper() == "lstm".upper():
        printb("LSTM state", printable=self.debug_mode)
        H_exp = tf.expand_dims(H[self.num_layers - 1][0], 1)
    
    elif self.rnn_cell_type.upper() == "gru".upper():
        printb("GRU state", printable=self.debug_mode)
        #H_exp = tf.expand_dims(H, 1)
        H_exp = tf.expand_dims(H[self.num_layers - 1], 1)
    ...
    score = tf.matmul(H_exp, self.W(EO), transpose_b=True)
